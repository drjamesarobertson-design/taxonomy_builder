...
1. **Optional/multiple delimiters with a setup wizard.** Done: the single fixed `delimiterAfter` setting is replaced with `delimiterPositions: number[]`, so the taxonomy can have zero, one, or several "-" delimiter columns. The New Taxonomy form gained a "Code Delimiters" section — a dynamic list of "Insert '-' Code Delimiter — after how many code columns?" rows, each with an Add/Remove control and an empty-state message when none are configured — as the web-form equivalent of James's described sequential Y/N prompt flow (sequential native prompts were already found unreliable against buffered keystrokes elsewhere in this project). Older saved project files that still carry the old `delimiterAfter` field are migrated to `delimiterPositions` transparently on load. Verified: creating a taxonomy with zero delimiters renders no delimiter column at all; the default single delimiter (after column 3) matches prior behaviour; configuring delimiters after both columns 3 and 6 renders both correctly in the header and every body row; loading James's own older sample file (which uses `delimiterAfter`) still loads and renders its delimiter correctly. Catering for more than 8 columns as a setup option remains noted as future scope, not part of this piece of work.
2. **Shift-select and delete a range of codes.** Done: the selection mechanism (click/shift-click/ctrl-click) is now shared between code and description cells rather than description-only, tracking which kind of cell is selected alongside the level and row set. Right-clicking a code selection shows a context menu with "Delete" (right-clicking a description selection still shows Toggle Case/Promote/Demote as before). Delete clears every selected code cell and, for each affected row, every code to its right too — the same rule as any other code change (Section 4.4/6.3). Verified: shift-click range-selects code cells with the same visual highlight as descriptions; Delete clears exactly the selected cells and leaves untouched rows alone; deleting a shallower code also clears a row's deeper ones; the description-side selection, Toggle Case, and Promote/Demote all still work correctly after sharing the underlying selection state.
...
### Second grid-editing refinement round

Another feedback pass after items 1–3 above shipped:

- **Maximum ERP Description Field Length input.** The field was coerced to a number on every keystroke, so backspacing it to empty snapped straight to a stray "0" that then sat in front of the next digit typed. Now kept as free text while editing and only parsed/clamped on blur or submit.
- **Delimiter setup form layout.** The "Remove" button sat immediately next to the position number, which read as ambiguous. Moved to its own row below the input.
- **New taxonomy starts with a row in place.** Creating a taxonomy previously left an empty grid requiring a click to begin. It now starts with one row already added, cursor focused on its first description cell.
- **"." padding cascade, corrected fully.** Previously, when "." replicated rightward across several columns on the edited row, the downward cascade for ALL of those columns was tied to a single shared stop condition (effectively the leftmost column's). If that column hit a non-blank cell early, every other replicated column stopped cascading too, even where they were still blank further down. Each replicated column now cascades down independently, stopping only when that specific column meets a non-blank cell.
- **Promote/Demote scope, corrected.** Previously blanked every code cell on each affected row. Now only the code cell at the entry's *new* column is blanked — the one whose value is no longer trustworthy at the new level. The rest of the row's codes (its ancestor path) are left alone, so a promote/demote no longer looks like it wiped the whole row.
- **Row insert/delete (Section 6.5), previously missing entirely.** Right-click any cell for "Insert Row Above", "Insert Row Below", or "Delete Row", each relative to the row the context menu was opened on (not a global add-to-bottom). Deleting a row with children now shows a confirm dialog ("this entry has child entries — deleting it will delete its children too") before proceeding, per the spec's warning requirement; a childless row deletes immediately. The existing per-row "×" button now goes through the same warn-if-children path instead of deleting silently.

Verified with targeted new Playwright coverage for each fix (including a case specifically constructed so a shallower replicated column stops at a real value while deeper columns keep cascading past it) plus a full re-run of the accumulated regression suite — no regressions.

James also asked how difficult adding Undo (Section 6.8) would be — answered as a moderate, well-contained piece of work given the grid's single centralized `rows` array, not yet built.

### Third feedback round — branding, and the remaining Stage 3/6.5 features

A larger batch (refinements plus new functionality), split into three shippable tranches per James's suggestion. Undo coalescing-per-field was confirmed as the eventual design but explicitly held back for later, not built this round.

**Tranche 1 — branding and sign-on screen cosmetics.** Both the sign-on and working-grid screens now show "The ERP Doctor Taxonomy Builder" as the heading (36pt sans-serif, left-justified; the rest of the UI, including the "Create a New Taxonomy" subheading, keeps its existing monospace font and size, changing only to sans-serif for that one subheading). A placeholder box reserves space for the logo to the heading's right until James supplies the actual image file (not retrievable from the chat attachment itself — needs a real file upload). The sign-on screen only gets a dark blue background (`#102a5c`) with light text, toggled via a body class so it covers the full page without touching the working grid's existing light theme. Added tooltips (native `title` attribute) to Title, Table Name, Purpose, and Maximum ERP Description Field Length, and the copyright footnote, both sign-on-screen only. Moved "Load from File" out of the header and next to "Create Taxonomy" on the sign-on screen (still in the header once a taxonomy is open, for loading a different file without discarding first). Also fixed the Maximum ERP Description Field Length input snapping to a stray "0" when backspaced to empty.

**Tranche 2 — Replicate Codes Above, and single-entry-vs-children promote/demote.** Right-click "Replicate Codes Above" on a code cell (or a selected range) copies the code from the row directly above into every selected cell in that column — for recovering after codes were deleted — deliberately skipping the ascending-order check since it's a copy, not a fresh entry; warns if there's nothing above to copy. Promote/Demote now asks "just this entry, or this entry and its children?" whenever the selection includes an entry that actually has children (skipped for a childless entry, which still moves immediately); "Just This Entry" moves only the selected row(s), deliberately leaving children in place, while "Entry + Children" is the existing whole-block behaviour. A boundary violation on the entry itself (already at the leftmost/rightmost level) is still reported immediately, before ever asking about scope.

**Tranche 3 — drag-select bulk row insert, and Move.** Click-and-drag now extends a row selection down a column the same way shift-click does (Section 6.5's "drag cursor down column to highlight range"); right-clicking within a multi-row selection turns "Insert Row Above/Below" into "Insert N Rows Above/Below", relative to the top/bottom of the whole selected range rather than wherever inside it was right-clicked. Right-click a description cell and choose "Move" to relocate an entry and all of its descendants elsewhere in the list: a banner confirms move mode and how many rows are selected, the moving rows are dimmed, and the next plain click on any other row offers "Insert Above" / "Insert Below" relative to that target (Escape cancels; clicking one of the rows being moved itself is rejected). It's a pure reorder — the moved rows' own descriptions and codes are left exactly as they were.
...
### Fifth feedback round — Override, Insert Row duplication, a real drag bug, headings, and Export

- **Ascending-order Override.** A code that would break ascending order (typically: decreasing a value the user had already moved past) now offers "Codes should increase, lesser value is invalid—Override?" with an Override button, instead of just blocking with OK. Overriding re-applies the exact same edit, skipping only the order check — charset, left-to-right population, and the description-hierarchy bound still apply.
- **Insert Row Above/Below now duplicates codes.** Previously left both codes and description blank; now duplicates codes from whichever row ends up directly above the new one(s) (same convention as Add Row), leaving only the description blank for the user to type.
- **A genuine bug in last round's multi-column Replicate Codes Above.** James reported the multi-column selection "only replicates for the row and cell where the cursor is" — investigation found the feature was correctly implemented and shipped, but unreliable in real use: code cells are only 1 character wide, and a real (non-synthetic) fast mouse drag can skip straight over one without its mouseenter event ever firing, especially dragging sideways across several to form a block. Fixed by also running selection extension off a window-level `mousemove` listener that re-derives the cell under the cursor via `elementFromPoint` on every move — this doesn't depend on "entering" any single narrow target the way per-cell mouseenter does, so it keeps up regardless of drag speed. Confirmed with a Playwright test using a single unstepped mouse jump (no interpolated intermediate positions) between two distant cells, which reproduced the failure before the fix and passes after.
- **"Code" / "Description" section headings** added above the grid's column-number row, in the app's dark blue with white text.
- **CSV and Excel export, both Discrete Columns and Concatenated (Stage 5, Section 7, plus a Section 9 item James asked to bring forward).** "Export to CSV" and "Export to Excel" now open a small choice dialog. **Discrete Columns** produces the raw grid exactly as it appears on screen — one column per code position (delimiter included as a literal "-"), one column per description level. **Concatenated** produces one combined Code column and one combined Description column per posting-level row, for feeding an ERP import — since CLAUDE.md Section 9 left its exact format undecided, James was asked three clarifying questions (how much of the code column width to keep, what indent character to use, whether the description should repeat ancestor text) and answered "no preference" to all three, so the recommended defaults were used: the code is trimmed to the row's own significant depth (e.g. "1","2","3" becomes "123", not padded to the full column width), the description is the row's own text only (no repeated ancestor breadcrumb) indent-padded with one leading space per level above the top, and rows with no description at all are skipped. The Excel export (via `exceljs`, lazy-loaded so it doesn't add to everyone's initial bundle) also carries the column colour-coding and a bold header row as real cell formatting for Discrete Columns.

Verified with new Playwright coverage for each item, including parsing the exported XLSX back with `exceljs` to confirm cell values, the bold header, and per-column fill colours; re-ran the full accumulated regression suite — no regressions.
