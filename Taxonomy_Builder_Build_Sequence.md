...
- **CSV and Excel export, Discrete Columns mode (Stage 5, Section 7).** "Export to CSV" and "Export to Excel" in the grid toolbar produce the raw grid exactly as it appears on screen — one column per code position (delimiter included as a literal "-"), one column per description level. The Excel export (via `exceljs`, lazy-loaded so it doesn't add to everyone's initial bundle) also carries the column colour-coding and a bold header row as real cell formatting. James also asked for a "Concatenated" mode for both formats — not built yet, since CLAUDE.md Section 9 explicitly left its format (delimiter/padding substitution in the code, how much indent-padding descriptions get) undecided, and a wrong guess produces a file meant to feed straight into an ERP import; the specifics are being confirmed with James before building it.
