...
//
// James's rule, restated after two rounds of clarification: within any sibling group (rows
// sharing the same immediate parent — a fresh "start at 1" every group, never a running total
// down the whole column), spread codes evenly across 1-9 so the first sibling always lands on
// "1" and the last always lands on "9" (even with as few as two siblings) — "0" is never used —
// extending into capital letters ("A", "B", "C"...) only if a group genuinely has more than 9
// members. The SAME rule applies at every level (his own correction — no separate, tighter rule
// for the deepest column). Once every row's own-level code is assigned, each ancestor's code is
// carried down through its descendants' blank ancestor columns, and every column deeper than a
// row's own level is padded — exactly the wizard's existing Fill Codes / Pad Codes steps, reused
// here rather than reimplemented, run automatically as part of the one action.

import type { TaxonomyRow } from './types';
import { fillCodesDown, maxLevelUsed, padCodes } from './guidance';
...
// Evenly spreads `count` siblings across the first 9 slots ("1".."9") so the first always lands
// on index 0 ("1") and the last always lands on index 8 ("9") — proportional rounding in
// between, not a fixed step, so both ends are reached exactly regardless of how many siblings
// there are. Beyond 9 siblings there's no spare room left to space out at all — that's already
// past the taxonomy's own 5-9 recommendation — so the scheme just keeps counting through the
// alphabet with no gaps rather than trying to spread further.
function spreadSlots(count: number): string[] {
  ...
  for (let i = 0; i < count; i++) {
    positions.push(Math.round((i * 8) / (count - 1)));
  }
...
// siblings so nothing collides with it.
